"""
月次助成金リサーチ結果を ChatWork に投稿するスクリプト。

使い方:
    python scripts/notify_chatwork.py data/grants/2026-05.md

環境変数:
    CHATWORK_API_TOKEN  ChatWork APIトークン
    CHATWORK_ROOM_ID    投稿先ルームID
"""

import os
import re
import sys
import logging
import time
from pathlib import Path

import requests

logging.basicConfig(level=logging.INFO, format="%(levelname)s: %(message)s")
logger = logging.getLogger(__name__)

CHATWORK_API_BASE = "https://api.chatwork.com/v2"


# ---------------------------------------------------------------------------
# ChatWork API
# ---------------------------------------------------------------------------

def post_message(token: str, room_id: str, body: str) -> str | None:
    """ChatWork にメッセージを投稿し、message_id を返す。"""
    url = f"{CHATWORK_API_BASE}/rooms/{room_id}/messages"
    headers = {"X-ChatWorkToken": token}
    data = {"body": body, "self_unread": "1"}

    resp = requests.post(url, headers=headers, data=data, timeout=30)
    resp.raise_for_status()
    result = resp.json()
    message_id = result.get("message_id")
    logger.info("投稿成功 message_id=%s", message_id)
    return str(message_id) if message_id else None


def post_error_notice(token: str, room_id: str, file_path: str, error: str) -> None:
    """パース失敗時にエラー通知を投稿する。"""
    body = (
        "[info][title]【助成金BOT】処理エラー[/title]"
        f"ファイル: {file_path}\n"
        f"エラー: {error}\n\n"
        "リサーチ結果ファイルの読み取りに失敗しました。\n"
        "ファイルの形式を確認してください。[/info]"
    )
    post_message(token, room_id, body)


# ---------------------------------------------------------------------------
# Markdown パーサー
# ---------------------------------------------------------------------------

def parse_header(content: str) -> dict:
    """ヘッダーから年月とメタ情報を抽出する。"""
    header = {}
    # タイトル行から年月を取得
    m = re.search(r"月次助成金スキャン[｜|](\d{4})年(\d{1,2})月", content)
    if m:
        header["year"] = m.group(1)
        header["month"] = m.group(2).zfill(2)
    else:
        header["year"] = "?????"
        header["month"] = "??"
    return header


def parse_candidates(content: str) -> list[dict]:
    """区分1（採否候補）の各候補をパースする。"""
    candidates = []

    # 区分1セクションを抽出
    section1_match = re.search(
        r"##\s*区分1[：:]?\s*今月の採否候補(.*?)(?=##\s*区分[2-5]|\Z)",
        content,
        re.DOTALL,
    )
    if not section1_match:
        return candidates

    section1 = section1_match.group(1)

    # 0件月チェック
    if re.search(r"今月の採否候補[：:]\s*0件", section1):
        return candidates

    # ### で始まる各候補ブロックを分割（上段/下段ではなく候補単位で分割）
    # 候補は "### 上段" or 候補名のヘッダーで区切られる
    # 柔軟にパースするため、主要フィールドを正規表現で抽出する
    candidate_blocks = re.split(r"\n(?=###\s)", section1)

    current = {}
    for block in candidate_blocks:
        # 上段（会議用判断カード）のフィールドを抽出
        fields = {
            "制度名": r"制度名[：:]\s*(.+)",
            "想定使途": r"想定使途[（(].*?[）)][：:]\s*(.+)|想定使途[：:]\s*(.+)",
            "締切": r"締切[（(].*?[）)][：:]\s*(.+)|締切[：:]\s*(.+)",
            "準備余裕": r"準備余裕[（(].*?[）)][：:]\s*(.+)|準備余裕[：:]\s*(.+)",
            "金額感": r"金額感[（(].*?[）)][：:]\s*(.+)|金額感[：:]\s*(.+)",
            "事業適合": r"事業適合[（(].*?[）)][：:]\s*(.+)|事業適合[：:]\s*(.+)",
            "仮判断ラベル": r"仮判断ラベル[（(].*?[）)][：:]\s*(.+)|仮判断ラベル[：:]\s*(.+)",
            "一言でいうと": r"一言でいうと[：:]\s*(.+)",
        }

        for key, pattern in fields.items():
            m = re.search(pattern, block)
            if m:
                # 複数グループのうち最初にマッチしたものを使う
                value = next((g for g in m.groups() if g), None)
                if value:
                    current[key] = value.strip()

        # リスト形式のフィールド
        # 次のフィールド名（採る理由/実行条件/主な負担・制約/罠チェック/不明点）で区切る
        list_stop = r"(?=[-\-]\s*(?:採る理由|実行条件|主な負担・制約|罠チェック|不明点)[：:]|###|【|\Z)"
        list_fields = {
            "採る理由": rf"採る理由[：:]?(.*?){list_stop}",
            "実行条件": rf"実行条件[：:]?(.*?){list_stop}",
            "主な負担・制約": rf"主な負担・制約[：:]?(.*?){list_stop}",
        }

        for key, pattern in list_fields.items():
            m = re.search(pattern, block, re.DOTALL)
            if m:
                # フィールド名自体を含む行を除外してリスト項目だけ取る
                raw = m.group(1)
                items = [
                    line.strip()
                    for line in re.findall(r"[-・]\s*(.+)", raw)
                    if not re.match(r"(?:採る理由|実行条件|主な負担・制約)[：:]", line.strip())
                ]
                if items:
                    current[key] = items

        # 制度名が見つかったらこのブロックは新しい候補の開始
        if "制度名" in current and current not in candidates:
            # 既に同じ制度名の候補があれば統合、なければ追加
            existing = next(
                (c for c in candidates if c.get("制度名") == current["制度名"]),
                None,
            )
            if existing:
                existing.update(current)
            else:
                candidates.append(current)
                current = {}

    # 最後の候補を追加
    if current and current not in candidates:
        existing = next(
            (c for c in candidates if c.get("制度名") == current.get("制度名")),
            None,
        )
        if existing:
            existing.update(current)
        elif current.get("制度名"):
            candidates.append(current)

    return candidates


# ---------------------------------------------------------------------------
# ChatWork メッセージ組み立て
# ---------------------------------------------------------------------------

def build_individual_message(index: int, candidate: dict) -> str:
    """個別通知（会議用判断カード）を組み立てる。"""
    number = chr(0x2460 + index)  # ①②③...
    title_name = candidate.get("制度名", "不明な制度")
    usage = candidate.get("想定使途", "")
    if usage:
        title_name += f"｜{usage}"

    deadline = candidate.get("締切", "未確認")
    readiness = candidate.get("準備余裕", "未確認")
    money = candidate.get("金額感", "未確認")
    label = candidate.get("仮判断ラベル", "要確認")
    summary = candidate.get("一言でいうと", "")

    lines = [
        f"[info][title]{number} {title_name}[/title]",
        f"■ 想定使途: {candidate.get('想定使途', '未確認')}",
        f"■ 締切/準備余裕: {deadline} / {readiness}",
        f"■ 金額感: {money}",
        f"■ 今日の仮判断: {label}",
        "",
    ]

    if summary:
        lines.append(f"【一言でいうと】\n{summary}")
        lines.append("")

    # 採る理由
    reasons = candidate.get("採る理由", [])
    if reasons:
        lines.append("【採る理由】")
        for r in reasons:
            lines.append(f"- {r}")
        lines.append("")

    # 実行条件
    conditions = candidate.get("実行条件", [])
    if conditions:
        lines.append("【実行条件】")
        for c in conditions:
            lines.append(f"- {c}")
        lines.append("")

    # 主な負担・制約
    burdens = candidate.get("主な負担・制約", [])
    if burdens:
        lines.append("【主な負担・制約】")
        for b in burdens:
            lines.append(f"- {b}")
        lines.append("")

    lines.extend([
        "【会議で決めること】",
        "□ 採用",
        "□ 条件付き採用",
        "□ 持越し",
        "□ 見送り",
        "",
        "担当候補: ",
        "次期限: ",
        "[/info]",
    ])

    return "\n".join(lines)


def build_summary_message(
    header: dict,
    candidates: list[dict],
    message_ids: list[str | None],
    room_id: str,
) -> str:
    """サマリ通知（議事進行表）を組み立てる。"""
    year = header.get("year", "????")
    month = header.get("month", "??")
    count = len(candidates)

    lines = [
        f"[info][title]【月次助成金スキャン｜{year}年{month}月｜本日{count}件】[/title]",
        "本日の役員会議の冒頭議題です。上から順に読みます。",
        "",
    ]

    # 仮判断ラベルでグルーピング
    groups = {
        "A. 本日判断したい": [],
        "B. 情報追加後に判断したい": [],
        "C. 速決候補": [],
        "その他": [],
    }

    label_to_group = {
        "条件付き採用": "A. 本日判断したい",
        "要確認": "A. 本日判断したい",
        "持越し候補": "B. 情報追加後に判断したい",
        "採用候補": "C. 速決候補",
        "見送り候補": "その他",
    }

    for i, c in enumerate(candidates):
        label = c.get("仮判断ラベル", "要確認")
        group_key = label_to_group.get(label, "その他")
        groups[group_key].append((i, c))

    for group_name, items in groups.items():
        if not items:
            continue
        lines.append(group_name)
        for i, c in items:
            number = chr(0x2460 + i)
            name = c.get("制度名", "不明")
            usage = c.get("想定使途", "")
            label = c.get("仮判断ラベル", "要確認")
            readiness = c.get("準備余裕", "未確認")

            display_name = f"{name}｜{usage}" if usage else name
            lines.append(f"{number} {display_name}")
            lines.append(f"　仮判断: {label} / 準備余裕: {readiness}")

            # 個別メッセージへのリンク
            mid = message_ids[i] if i < len(message_ids) else None
            if mid:
                link = f"[引用 aid=0 time=0]個別メッセージ: https://www.chatwork.com/#!rid{room_id}-{mid}[/引用]"
                lines.append(f"　→ {link}")
            lines.append("")

    lines.extend([
        "今日この場で決めること",
        "1. 採用 / 条件付き採用 / 持越し / 見送り",
        "2. 担当",
        "3. 次期限",
        "[/info]",
    ])

    return "\n".join(lines)


def build_zero_message(header: dict) -> str:
    """0件月の通知メッセージを組み立てる。"""
    year = header.get("year", "????")
    month = header.get("month", "??")
    return (
        f"[info][title]【月次助成金スキャン｜{year}年{month}月｜本日0件】[/title]"
        "今月の採否候補はありませんでした。\n\n"
        "Tier1 全件を巡回した結果、HAMAYOUの事業適合・準備余裕・"
        "Core Facts要件をすべて満たす制度は今月は確認できませんでした。\n\n"
        "監視継続している有望候補はリサーチ結果ファイルを参照してください。\n\n"
        "会議での扱い: 本月は新規議題なし。前月までの採用案件の進捗確認のみ。[/info]"
    )


# ---------------------------------------------------------------------------
# メイン処理
# ---------------------------------------------------------------------------

def main() -> None:
    if len(sys.argv) < 2:
        logger.error("使い方: python notify_chatwork.py <data/grants/YYYY-MM.md>")
        sys.exit(1)

    file_path = sys.argv[1]
    token = os.environ.get("CHATWORK_API_TOKEN", "")
    room_id = os.environ.get("CHATWORK_ROOM_ID", "")

    if not token or not room_id:
        logger.error("CHATWORK_API_TOKEN と CHATWORK_ROOM_ID を設定してください")
        sys.exit(1)

    # ファイル読み込み
    path = Path(file_path)
    if not path.exists():
        logger.error("ファイルが見つかりません: %s", file_path)
        sys.exit(1)

    content = path.read_text(encoding="utf-8")

    # パース
    try:
        header = parse_header(content)
        candidates = parse_candidates(content)
    except Exception as e:
        logger.error("パースエラー: %s", e)
        post_error_notice(token, room_id, file_path, str(e))
        sys.exit(1)

    # 0件月
    if not candidates:
        msg = build_zero_message(header)
        post_message(token, room_id, msg)
        logger.info("0件月の通知を投稿しました")
        return

    # 個別通知を投稿
    message_ids: list[str | None] = []
    for i, candidate in enumerate(candidates):
        msg = build_individual_message(i, candidate)
        mid = post_message(token, room_id, msg)
        message_ids.append(mid)
        # レートリミット対策（念のため）
        if i < len(candidates) - 1:
            time.sleep(1)

    # サマリ通知を投稿
    time.sleep(1)
    summary = build_summary_message(header, candidates, message_ids, room_id)
    post_message(token, room_id, summary)

    logger.info("全%d件 + サマリの投稿が完了しました", len(candidates))


if __name__ == "__main__":
    main()
